...
app.post('/typeedit', (req, res) => {
    ...
    if(types == null){
        ...
        mysqlConnection.query('SELECT car.carType, carId, name, brandName, carStatus, location, rates.weekly, rates.hourly, rates.daily from car inner join rates on car.carType = rates.carType WHERE ( car.carType = ? OR car.carType = ? OR car.carType = ? OR car.carType = ? OR car.carType = ? OR car.carType = ? OR car.carType = ? OR car.carType = ? OR car.carType = ? )', [types[0],types[1],types[2],types[3],types[4],types[5],types[6],types[7],types[8]], (err, rows, fields) => {
            if (!err){
                if(res == null){
                    res.send("oops");
                }
